...
function binaryToHexadecimal(blob) {
  // let bin = parseInt(blob.slice(2), 2);
...
function hexadecimalToBinary(blob) {
  // let hex = parseInt(blob.slice(2), 16);
  // return`0b${hex.toString(2)}`;
...
function hexadecimalToAscii(blob) {
  let bin = parseInt(blob.slice(2), 16);
  return String.fromCharCode(bin);
}
...
function asciiToDecimal(blob) {
  ...
  // return blob.split('').map((_, index) => blob.charCodeAt(index))
}
